...
export function localStorage(namespace = '') {
  return {
    setItem(lastname = '', value = {}, seg = 14400) {
      ...
      Storage.setItem(`${namespace}${lastname}`, JSON.stringify({
        value: value,
        expires: expirarem
      }))
    },
    getItem(lastname) {
      ...
      let itemValue = Storage[`${namespace}${lastname}`]
    ...
    getAll() {
      ...
      for (let i = 0, len = Storage.length; i < len; ++i) {
        ...
        if (this.namespace === itemKey.substring(0, namespace.length)) {
    ...
    removeItem(lastname) {
      ...
      Storage.removeItem(`${namespace}${lastname}`)
    },
    clear() {
      //Remove tudo
      Storage.clear()
