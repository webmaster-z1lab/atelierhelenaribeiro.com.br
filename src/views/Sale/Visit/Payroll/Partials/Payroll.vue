...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      async submitForm() {
        if (!isEmpty(this.products)) {
          this.EDIT_PAYROLL({visit_id: this.visit.id, ext: '', data: this.products})
            .then(response => {
              notifyVue(this.$notify, 'Consignado criado com sucesso', 'success');
            })
    ...
    mounted() {
      this.products = this.visit.payrolls
    }
  };
</script>
