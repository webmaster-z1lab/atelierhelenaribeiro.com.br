  ...
  <div class="container-fluid mt--6" v-else>
    <div class="row">
      ...
      <div class="col-12">
          ...
          <div class="form-row">
            ...
            <div class="col-lg-12" v-if="payment_method === 'check'">
              <div class="form-row" v-for="(item, key) in data_check" :key="item">
                ...
                <div class="col">
                  <base-input name="name_check" label="Nome do Titular" v-model="item.name_check" :error="getError('amount')" :valid="isValid('amount')" v-validate="'required|min_value:1'"/>
                </div>
                <div class="col">
                  <base-input name="number_check" label="Nº do Cheque" v-model="item.number_check" :error="getError('amount')" :valid="isValid('amount')" v-validate="'required|min_value:1'"/>
                </div>
                <div class="col">
                ...
                <div class="col">
                  <base-input name="bank_check" label="Banco" v-model="item.bank_check" :error="getError('amount')" :valid="isValid('amount')" v-validate="'required|min_value:1'"/>
                </div>
                <div class="col" v-if="installments_method > 1">
                  <money-input label="Valor Total" v-model="item.value_method" :key="key" name="value" :error="getError('value')" :valid="isValid('value')" v-validate="'required|min_value:1'"/>
                </div>
              </div>
            </div>
          </div>
        ...
        <card>
          <h3  slot="header" class="mb-0">Finalizando Visita</h3>

          <div class="form-row">
            <div class="col-lg-3 card bg-gradient-primary mr-1" v-if="!emptyPaychecks">
              <!-- Card body -->
              <div class="card-body">
                <div class="row justify-content-between align-items-center">
                  ...
                  <div class="col-auto">
                    <span class="badge badge-lg badge-success">{{translateMethod('check')}}</span>
                  </div>
                </div>
                <div class="my-4">
                ...
                <div class="row">
                  <div class="col">
                    ...
                    <span class="d-block h3 text-white">{{1}}</span>
                  </div>
                </div>
              </div>
            </div>
            <div class="col-lg-3 card bg-gradient-primary mr-1" v-for="method in data_methods.payment_methods">
              <!-- Card body -->
              <div class="card-body">
                <div class="row justify-content-between align-items-center">
                  <div class="col">
                    <i class="fas fa-file-invoice-dollar fa-2x text-white" v-if="method.method === 'boleto'"></i>
                    <i class="far fa-credit-card fa-2x text-white" v-if="method.method === 'credit_card'"></i>
                    <i class="fas fa-money-bill fa-2x text-white" v-if="method.method === 'money'"></i>
                  </div>
                  <div class="col-auto">
                    <span class="badge badge-lg badge-success">{{translateMethod(method.method)}}</span>
                  </div>
                </div>
                <div class="my-4">
                  <span class="h6 surtitle text-light">
  ...
  export default {
    ...
    methods: {
      ...
      addPaymentMethod() {
        if (this.payment_method === 'check') {
          ...
          this.data_methods.payment_methods.push({
            ...
            instalments: this.installments_method,
      ...
      removePaymentMethod(key) {
        this.payment_methods.splice(key, 1)
      },
      async submitForm() {
        try {
